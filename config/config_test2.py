...
_LAYER_CONFIGS = {
    'L1': {
        ...
        'poisson_inputs': {
            'E': [
                {'target': 'gE',  'weight': 'EXT_E', 'N': 40},  
                {'target': 'gPV', 'weight': 'EXT_I', 'N': 40},  
            ],
            'PV': [
                {'target': 'gE',  'weight': 'EXT_E', 'N': 25},
                {'target': 'gPV', 'weight': 'EXT_I', 'N': 25},
            ],
            'SOM': [
                {'target': 'gE',  'weight': 'EXT_E', 'N': 25},
                {'target': 'gSOM','weight': 'EXT_I', 'N': 25},
            ],
            'VIP': [
                {'target': 'gE',  'weight': 'EXT_E', 'N': 40},
                {'target': 'gPV', 'weight': 'EXT_I', 'N': 40},
    ...
    'L23': {
        ...
        'poisson_inputs': {
            'E': [
            {'target': 'gE',  'weight': 'EXT_E', 'N': 300, 'rate': 12*Hz}, 
            {'target': 'gPV', 'weight': 'EXT_I', 'N': 300, 'rate': 5*Hz}, 
            ],
            'PV': [
                {'target': 'gE',  'weight': 'EXT_E', 'N': 200, 'rate': 12*Hz},
                {'target': 'gPV', 'weight': 'EXT_I', 'N': 200, 'rate': 5*Hz},
            ],
            'SOM': [
                {'target': 'gE',   'weight': 'EXT_E', 'N': 200, 'rate': 12*Hz},
                {'target': 'gSOM', 'weight': 'EXT_I', 'N': 200, 'rate': 5*Hz},
            ],
            'VIP': [
                {'target': 'gE',  'weight': 'EXT_E', 'N': 200, 'rate': 12*Hz},
                {'target': 'gPV', 'weight': 'EXT_I', 'N': 200, 'rate': 5*Hz},
...
# --- Mapping from CSV rows/cols to populations --------------------------------
...
def _prob(src_row, tgt_col):
    try:
        return conn_df.loc[src_row, tgt_col]
    except KeyError:
...
def _cond(src_row, tgt_col):
    try:
        return conductances.loc[src_row, tgt_col]
    except KeyError:
...
# Fill per-layer and inter-layer connection probabilities and conductances -----
    ...
    for dst in _layers:
        ...
        for src_pop in _pops:
            for dst_pop in _pops:
                ...
                col  = f'{dst_pop}_col'
                if src == dst:
                    _LAYER_CONFIGS[src]['connection_prob'][conn] = _prob(s[row], t[col])
                    _LAYER_CONFIGS[src]['conductance'][conn] = _cond(s[row], t[col])
                else:
                    _INTER_LAYER_CONNECTIONS[(src, dst)][conn]  = _prob(s[row], t[col])
                    _INTER_LAYER_CONDUCTANCES[(src, dst)][conn] = _cond(s[row], t[col])

# Scale conductances for inhibitory presynaptic cells within layers -----------

for layer in ['L23', 'L4', 'L5', 'L6']:
    cfg = _LAYER_CONFIGS[layer]
    for conn_type in list(cfg['conductance'].keys()):
        pre, _ = conn_type.split('_')
        if pre == 'PV':
...
# Scale inter-layer conductances (leave as you had) ---------------------------

for (src, dst), conds in _INTER_LAYER_CONDUCTANCES.items():
    for conn_type in list(conds.keys()):
        pre, _ = conn_type.split('_')
        if pre == 'PV':
            _INTER_LAYER_CONDUCTANCES[(src, dst)][conn_type] *= 1.2
        if pre == 'E':
            _INTER_LAYER_CONDUCTANCES[(src, dst)][conn_type] *= 0.8

# E-E slightly reduced as before ----------------------------------------------
...
CONFIG = {
    ...
    'models': {
        'equations': {
            'E': """
        dv/dt = (gL*(EL - v) + gL*DeltaT*exp((v - VT)/DeltaT)
                 + gE*(Ee - v) - (gPV + gSOM + gVIP)*(v - Ei)
                 - w + I)/C : volt (unless refractory)
        IsynE   = gE*(Ee - v) : amp
    ...
    'intrinsic_params': {
        # *** softened adaptation on E ***
        'E':   {'a': 2*nS, 'b': 40*pA, 'DeltaT': 2*mV,
                'C': 97*pF, 'gL': 4.2*nS, 'tauw': 120*ms, 'EL': -66*mV},
    ...
    'initial_conditions': {
        'DEFAULT': {'v': '-60*mV + rand()*8*mV',
                    'gE': 0*nS, 'gPV': 0*nS, 'gSOM': 0*nS, 'gVIP': 0*nS,
